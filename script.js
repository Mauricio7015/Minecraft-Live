    ...
    const BLOCK_SIZE = 45;
    const NUM_BLOCKS = 8;
    const URL_LIVE = "https://www.youtube.com/live/MlOpWnivxro"; // URL do vídeo ao vivo do YouTube
    let currentPickaxeType = 'iron'; // ou 'iron'
    ...
    Events.on(engine, 'collisionStart', event => {
      event.pairs.forEach(pair => {
        blocks.forEach((block, index) => {
          ...
          // só aplicar dano se quem bateu foi a picareta
    ...
    // Impulso para cima se picareta travar
    ...
    function spawnBlocksIfNeeded() {
      if (pickaxe.position.y > lastY - 300) {
        ...
        document.body.style.height = `${lastY + 600}px`;
        updateCounter();
      }
    ...
    function explodeTNT(tnt) {
      ...
      // Remove corpo físico e visual da TNT
      ...
      // Destroi blocos próximos e contabiliza
      ...
      updateCounter(); // atualiza contadores visuais
    ...
    function insertTntBlockAbovePickaxe() {
      ...
      tntEl.className = 'block tnt blinking'; // começa piscando
    ...
    async function getLikes() {
      if (processing) return; // opcional, evita chamadas concorrentes

      const formdata = new FormData();
      formdata.append("action", "aux-youtube-live-likes-comments");
      formdata.append("url", URL_LIVE);
      ...
      try {
        const response = await fetch("https://auxmode.com/wp-admin/admin-ajax.php", requestOptions);
        const result = await response.text();
        const data = JSON.parse(result);
        const newLikes = data.data.likes || 0;
      ...
      // Aguarda 5 segundos antes da próxima chamada
